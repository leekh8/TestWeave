...
- **보안 점검 모듈** (`SecurityCheck` 인터페이스 — 새 모듈은 구현체 추가만으로 자동 인식)
  - `HeaderCheck` — HSTS · CSP · X-Frame-Options · X-Content-Type-Options · Referrer-Policy 존재 (HSTS는 `max-age>0`까지 검증 — `max-age=0`은 비활성으로 FAIL)
  - `CookieCheck` — `Set-Cookie`의 Secure / HttpOnly / SameSite 플래그 (속성 토큰 파싱 — 쿠키 값 속 문자열 오탐 방지)
  - `TlsCheck` — HTTPS 사용 여부 · **호스트네임(SAN/CN) 검증** · 서버 인증서 만료 임박(임계 14일)
